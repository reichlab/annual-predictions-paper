...
pi_best <- filter(pi_dat, num_cov==final_model_covs)
outbreak_roc <- pROC::roc(pi_best$outbreak_observed,
                          pi_best$outbreak_probability,
                          ci=T)

pi_other <- filter(pi_dat, num_cov!=final_model_covs)
outbreak_roc_other <- pROC::roc(pi_other$outbreak_observed,
                                 pi_other$outbreak_probability,
                                 ci=T)
...
<<annual-cases-graph, echo=FALSE, warning=FALSE, message=FALSE, fig.lp='fig:', fig.cap="Aggregated time series of dengue hemorrhagic fever cases from 2000-2014.">>=
thai_month_dhf <- monthly_counts %>%
    filter(date_sick_year>=FIRST_YEAR,
           date_sick_year<=LAST_YEAR) %>%
    group_by(date_sick_year, date_sick_month) %>%
    summarise(count=sum(count)) %>%
    mutate(date_sick=ymd(paste(date_sick_year, date_sick_month, "01", sep = "-")))

ggplot(data=thai_month_dhf) +
    geom_line(aes(x=date_sick, y=count)) +
    scale_y_continuous("Monthly DHF cases") +
...
incidence_heat_map <- ggplot(prov_counts) +
    ...
    scale_y_discrete("Provinces sorted by median\nannual incidence rate") +
    ggtitle("a") +
    theme_bw() + xlab("") + ylab("") +
...
outbreak_plot <-
    ...
    scale_x_discrete("Forecasted outbreak probability, by quantile",
                     labels=c("0-0.01", "0.01-0.05", "0.05-0.25", "0.25-0.50", "0.50-1.00")) +
    scale_y_continuous("Proportion of\nprovince-years with outbreaks",
                       limits=c(0,100),
                       breaks=seq(0,100,25),
...
ggplot() +
    ...
    geom_line(aes(x=seq(0,1,length.out = 330),
              ...
              linetype="dashed") +
    # geom_text(aes(label=paste("AUC = ",
    #                           round(outbreak_roc_other$auc,3)),
    #               x=0.75,
    #               y=0.25)) +
    scale_x_continuous("1 - Specificity") +
    scale_y_continuous("Sensitivity") +
    scale_color_manual("Model",
                       breaks=c("Incidence-only",
                                "WIP"),
                       values=cbbPalette) +
...
<<rel-mae-province, echo=FALSE, warning=FALSE, message=FALSE, fig.lp='fig:', fig.cap="Geographic variation in model and performance by province. \\textbf{(a)} The best fitted model in the testing phase for each Thai province, which shows spatial patterns of performance. \\textbf{(b)} The relative mean absolute error of the forecasts for each province from the models in (a) over the baseline forecasts. Provinces with: less error than the baseline are blue, more error than the baseline are red, and equal to the baseline are white.">>=
## prepare data to make province-level performance maps
best_model_prov <- final_preds_sum %>%
    left_join(select(best_models, num_cov, model)) %>%
    select(year, pid, ae_pred, ae_baseline, model) %>%
    group_by(pid, model) %>%
    mutate(rel_mae=mean(ae_pred)/mean(ae_baseline)) %>%
    group_by(pid) %>%
    arrange(rel_mae) %>%
...
prov_model_map <- thai_map
prov_model_map@data <- prov_model_map@data %>%
    left_join(best_model_prov, by = c("gns_adm1" = "pid")) %>%
    mutate(ID_1 = dense_rank(woe_name))
thai_locs <- fortify(prov_model_map, region="ID_1")

best_prov_model_map <- ggplot(prov_model_map@data, aes(map_id=ID_1)) +
    geom_map(aes(fill=model),
             color = "black", map=thai_locs) +
    expand_limits(x = thai_locs$long, y = thai_locs$lat) +
    scale_fill_manual("Best\nfitted\nmodel",
                      breaks=c("Fitted",
                               "Simple"),
...
#### make a map of relative MAE for each province
color_diff <- ceiling(100*max(abs(log10(best_model_prov$rel_mae))))/100
color_max <- color_diff
color_min <- -color_diff
scale_max <- log10(floor(10*10^color_max)/10)
scale_min <- log10(ceiling(10*10^color_min)/10)

rel_mae_prov_map <- ggplot(prov_model_map@data, aes(map_id=ID_1)) +
    geom_map(aes(fill=log10(rel_mae)), color = "black", map=thai_locs) +
    expand_limits(x = thai_locs$long, y = thai_locs$lat) +
    scale_fill_gradient2(low=cbbPalette[6], mid="white", high=cbbPalette[7],
                         limits=c(-color_max, color_max),
                         breaks=c(seq(scale_min, 0, length.out = 3),
                                  seq(0, scale_max, length.out = 3)[-1]),
                         labels=round(10^c(seq(scale_min,
                                             0,
                                             length.out = 3),
                                           seq(0,
                                             scale_max,
...
gridExtra::grid.arrange(best_prov_model_map +
                            ggtitle("a"),
                        rel_mae_prov_map +
                            ggtitle("b"),
                        layout_matrix=lay)
